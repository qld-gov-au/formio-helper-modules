window.onload = function () {
    ...
    Formio.createForm(
        ...
        var pdfDownload = function (submissionData) {
            if(submissionData) {
                // check PDF download option is checked and PDF exist on forms.io for a particular form
                if(formio_config.pdf_download && submissionData && submissionData.metadata && submissionData.metadata.PostToAPIGateway && submissionData.metadata.PostToAPIGateway.DownloadUrl){
        ...
        var submitLoaderIcon = function(label){
            if($("button[name^='data[submit']").find('.fa-spin').length <= 0){
                $("button[name^='data[submit']").prop('disabled', true).addClass('disabled').append('<i class="fa fa-circle-o-notch fa-spin ml-1"></i>');
            }
        }
        ...
        wizard.on("applicationSubmit", function (data) {
            ...
            submitLoaderIcon();
            setTimeout(function (){
                $("button[name^='data[submit']").find('.fa-spin').remove()
                $("button[name^='data[submit']").removeClass('disabled').attr("disabled", false);
            }, 2200);
            wizard
                .submit()
                .then(function (submissionData) {
                    submitLoaderIcon();
                    if(pdfDownload(submissionData)){
